...
class StateStore:
    ...
    def load_latest_ledger_record(self) -> dict[str, Any] | None:
        row = self._conn.execute(
            "SELECT record_id, timestamp, event_type, "
            "previous_record_hash, record_hash, payload_json "
            "FROM ledger ORDER BY record_id DESC LIMIT 1"
        ).fetchone()
        if row is None:
            return None
        record: dict[str, Any] = {
            ...
            "timestamp": row[1],
            "event_type": row[2],
            "previous_record_hash": row[3],
            "record_hash": row[4],
        }
        record.update(json.loads(row[5]))
        return record
