...
export function entraAuth() {
  ...
  console.log(`[entra] tenant=${ENTRA_EXPECTED_TENANT_ID}`);
  console.log(`[entra] audience=${ENTRA_ACCEPTED_AUDIENCES ? ENTRA_ACCEPTED_AUDIENCES.join(", ") : "(any)"}`);
  console.log(
    `[entra] issuers=${ENTRA_EXPECTED_ISSUERS.length ? ENTRA_EXPECTED_ISSUERS.join(", ") : "(any)"}`
  );
  console.log(
  ...
  if (isEntraRS256) {
    console.log(`[entra] oidc_discovery=${ENTRA_OIDC_DISCOVERY_URL || "(derived from tenant)"}`);
    console.log(`[entra] jwks_uri=${ENTRA_JWKS_URI || "(not configured)"}`);
  }
  ...
  return async function entraAuthMiddleware(req, res, next) {
    ...
    try {
      ...
      req.entraAuth = {
        ...
        token_type: claims.appid ? "app_only" : "delegated",
