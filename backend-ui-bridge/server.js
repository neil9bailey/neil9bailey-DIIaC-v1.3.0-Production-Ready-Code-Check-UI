...
const allowedOrigins = new Set([
  ...(process.env.APP_ENV !== "production" ? ["http://localhost:5173", "http://127.0.0.1:5173"] : []),
...
app.use(cors({
  ...
  allowedHeaders: ["Content-Type", "x-role", "Authorization", "X-Correlation-ID"]
}));

// Correlation ID — traces a request across bridge → Python runtime
...
// P0.4: In-memory sliding-window rate limiter (no extra dependencies)
const rateLimitStore = new Map();
const RATE_LIMIT_WINDOW_MS = 60_000; // 1 minute
const RATE_LIMIT_MAX = parseInt(process.env.RATE_LIMIT_MAX, 10) || 60;

function rateLimiter(req, res, next) {
  // Identify by authenticated subject or IP
  const key = req.actor?.subject || req.ip || "unknown";
  const now = Date.now();
  ...
  if (entry.timestamps.length >= RATE_LIMIT_MAX) {
    ...
    return res.status(429).json({ error: "rate_limit_exceeded", retry_after_seconds: retryAfter });
  }
...
// Unauthenticated — used by load balancers and container orchestrators
app.get("/health", (_req, res) => {
  res.json({
    status: "ok",
    service: "diiac-bridge",
...
app.get("/ready", async (_req, res) => {
  ...
  try {
    pythonReachable = await probePythonBases();
  } catch (_e) { /* not reachable */ }
  const ready = pythonReachable;
  res.status(ready ? 200 : 503).json({
    ready,
    python_runtime: pythonReachable ? "reachable" : "unreachable",
    timestamp: new Date().toISOString(),
  });
});

/* ================= LEDGER (delegated to lib/ledger.js) ================= */

const ledger = createLedger(LEDGER_PATH);
const ensureLedger = ledger.ensureLedger;
const getLastHash = ledger.getLastHash;
const appendLedger = ledger.append;
...
app.post("/api/human-input",
  ...
  (req, res) => {
    // P0.1: Validate human-input payload before writing to disk
    const body = req.body;
    ...
    if (serialized.length > 512_000) {
      return res.status(400).json({ error: "payload_too_large", message: "Human input payload exceeds 500KB limit." });
    }
    fs.mkdirSync(HUMAN_INPUT_DIR, { recursive: true });
...
// P0.2: Sanitize user-supplied context before sending to LLM to mitigate prompt injection
function sanitizeLlmContext(context) {
  const sanitized = {};
  const allowedTopKeys = new Set([
    "title", "description", "business_case", "objectives", "constraints",
    "stakeholders", "budget", "timeline", "domain", "sector", "risk_appetite",
    "raw_text", "intent", "requirements", "scope", "justification",
    "vendor_list", "evaluation_criteria", "strategic_priorities",
  ]);
  for (const [key, value] of Object.entries(context)) {
    if (!allowedTopKeys.has(key)) continue;
    if (typeof value === "string") {
      // Strip control characters and limit field length
      sanitized[key] = value.replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, "").slice(0, 50_000);
    } else if (Array.isArray(value)) {
      sanitized[key] = value.slice(0, 100).map((v) =>
        typeof v === "string" ? v.replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, "").slice(0, 10_000) : v
      );
    } else {
      sanitized[key] = value;
    }
...
async function generateAI(context, reasoning_level, policy_level, provider = "ChatGPT") {
...
`;
  ...
  const activeClient = isCopilot ? copilotClient : openai;
  const activeModel = isCopilot ? COPILOT_MODEL : OPENAI_MODEL;

  if (!activeClient) {
    if (!LLM_STUB_ENABLED) {
      throw new Error(isCopilot
        ? "Copilot provider enabled but GITHUB_TOKEN is missing"
        : "LLM ingestion enabled but OPENAI_API_KEY is missing");
    }
    return {
      executive_summary: {
  ...
  // P0.2: Sanitize context before sending to LLM
  const safeContext = sanitizeLlmContext(context);

  if (isCopilot) {
    // GitHub Models API uses standard chat completions
...
// P0.5: Default timeout for all Python proxy fetch calls (30 seconds)
const PROXY_TIMEOUT_MS = parseInt(process.env.PROXY_TIMEOUT_MS, 10) || 30_000;
...
app.post("/govern/decision",
  ...
  async (req, res) => {
    ...
    try {
      ...
      log.error("governed_execution_failed", { error: err.message, correlationId: req.correlationId });
...
app.post("/api/llm-governed-compile", requireRole(["admin"]), async (req, res) => {
  try {
    ...
    const aiReport = await generateAI(context, reasoning_level, policy_level, provider);
    ...
    log.error("llm_governed_compile_failed", { error: err.message, correlationId: req.correlationId });
    return res.status(500).json({ error: "llm_governed_compile_failed" });
  }
});
...
app.get("/executions/:execution_id/reports/:file",
  ...
  (req, res, next) => {
    ...
    const execId = req.params.execution_id.replace(/[^a-zA-Z0-9\-]/g, "");
    const artefactsDir = path.join(DECISION_PACK_BASE, execId, "artefacts");
    const f = path.resolve(artefactsDir, fileName);

    // Prevent path traversal — resolved path must stay within artefactsDir
    if (!f.startsWith(path.resolve(artefactsDir))) return res.status(400).json({ error: "invalid_path" });
...
async function ensurePythonRuntime() {
  ...
  pyStarting = new Promise((resolve) => {
    // P0.3: Whitelist env vars — only pass what the Python runtime needs
    const pythonEnvAllowlist = [
      "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "TMPDIR",
      "APP_ENV", "FLASK_RUN_PORT", "FLASK_DEBUG",
      "ADMIN_API_TOKEN", "ADMIN_AUTH_ENABLED",
      "SIGNING_PRIVATE_KEY_PEM", "SIGNING_ENABLED", "SIGNING_KEY_ID",
      "STRICT_DETERMINISTIC_MODE",
      "DIIAC_DB_PATH", "DIIAC_LOG_LEVEL",
      "PYTHONPATH", "PYTHONDONTWRITEBYTECODE",
    ];
    const pythonEnv = {};
    for (const key of pythonEnvAllowlist) {
      if (process.env[key] !== undefined) pythonEnv[key] = process.env[key];
    }
    pythonEnv.STRICT_DETERMINISTIC_MODE = pythonEnv.STRICT_DETERMINISTIC_MODE || "true";
...
async function queryPython(pathname) {
  ...
  const r = await fetch(`${activePythonBase}${pathname}`, { headers, signal: AbortSignal.timeout(PROXY_TIMEOUT_MS) });
  const ct = r.headers.get("content-type") || "application/json";
...
app.get("/admin/status/containers", requireRole(["admin"]), (_req, res) => {
  try {
    // Hardcoded command — never accept shell commands from environment variables
...
app.get("/admin/integrations/health", requireRole(["admin"]), async (_req, res) => {
  try {
    ...
    const anyLlmConfigured = Boolean(openai) || Boolean(copilotClient);
    const activeModels = [
      ...(Boolean(openai) ? [OPENAI_MODEL] : []),
...
app.get("/admin/config/effective", requireRole(["admin"]), (_req, res) => {
  return res.json({
    ...
    llm: {
      ...
      openai: { model: OPENAI_MODEL, api_key_present: Boolean(process.env.OPENAI_API_KEY) },
...
const server = app.listen(PORT, "0.0.0.0", () => {
  log.info("DIIaC Governance Engine started", {
    port: PORT,
    auth_mode: getAuthMode(),
    entra_enabled: isEntraEnabled(),
  });
...
// P0.7: Graceful shutdown — let in-flight requests finish before exiting
function gracefulShutdown(signal) {
  log.info("Shutdown initiated", { signal });
  server.close(() => {
    log.info("HTTP server closed");
    if (pyProc && !pyProc.killed) {
      log.info("Stopping Python runtime");
      pyProc.kill("SIGTERM");
    }
    process.exit(0);
  });
