...
app.post("/govern/decision",
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.error(err);
      res.status(500).json({ error: "governed_decision_failed" });
    }
  }
);
...
app.post("/api/llm-governed-compile", requireRole(["admin"]), async (req, res) => {
  try {
    ...
    console.error("[llm-governed-compile]", err);
...
app.get("/executions/:execution_id/reports/:file",
  ...
  (req, res, next) => {
    ...
    const artefactsDir = path.join(DECISION_PACK_BASE, req.params.execution_id, "artefacts");
    const f = path.resolve(artefactsDir, path.basename(req.params.file));

    // Prevent path traversal — resolved path must stay within artefactsDir
    if (!f.startsWith(path.resolve(artefactsDir) + path.sep) && f !== path.resolve(artefactsDir)) return res.status(400).json({ error: "invalid_file_path" });
...
app.post("/api/intercept/request",
  ...
  async (req, res) => {
    try {
      ...
      console.error("[intercept/request]", err);
      return res.status(500).json({ error: "intercept_failed" });
    }
  }
);
...
app.post("/api/intercept/response",
  ...
  async (req, res) => {
    try {
      ...
      console.error("[intercept/response]", err);
      return res.status(500).json({ error: "response_record_failed" });
    }
  }
);
...
app.post("/api/intercept/approval",
  ...
  async (req, res) => {
    try {
      ...
      console.error("[intercept/approval]", err);
...
app.get("/admin/integrations/health", requireRole(["admin"]), async (_req, res) => {
  try {
    ...
    console.error("[integrations/health]", err);
    return res.status(500).json({ error: "integrations_health_failed" });
  }
});
...
app.get("/health", (_req, res) => {
  res.json({ status: "OK", service: "backend-ui-bridge", timestamp: new Date().toISOString() });
});
