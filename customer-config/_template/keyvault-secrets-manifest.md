# REPLACE_WITH_CUSTOMER_NAME — Key Vault Secrets Manifest

**Key Vault:** `REPLACE_WITH_KEY_VAULT_NAME`
**Resource Group:** `REPLACE_WITH_RESOURCE_GROUP`
**Region:** `REPLACE_WITH_AZURE_REGION`
...
| `diiac-entra-client-secret` | Entra secret | 12 months | Client secret for the DIIaC API app registration. Rotate via Entra portal. |
...
az keyvault secret set \
  --vault-name REPLACE_WITH_KEY_VAULT_NAME \
...
az keyvault secret set --vault-name REPLACE_WITH_KEY_VAULT_NAME \
  --name "diiac-admin-api-token" --value "$NEW_TOKEN"
# Re-pull secrets and restart stack
bash scripts/pull-keyvault-secrets.sh --customer REPLACE_WITH_CUSTOMER_ID
