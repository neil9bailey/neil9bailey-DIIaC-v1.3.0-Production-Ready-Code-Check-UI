...
└── Web Interface         http://localhost:5173   (the UI you'll use)
        │
        Secrets from ────────► .env file (manual now → Key Vault when enabled)
        Login via ────────────► Vendorlogic Entra ID (Microsoft SSO)
```

> **Secrets are managed via Azure Key Vault `kv-diiac-vendorlogic`.**
> Run `bash scripts/pull-keyvault-secrets.sh` (or PowerShell equivalent) before starting the stack.
> See [`KEY_VAULT_TRANSITION.md`](KEY_VAULT_TRANSITION.md) for the integration details and checklist.
...
|---|--------------|-------------|-------------|
| 1 | **Docker Desktop** running | Open Docker Desktop — the whale icon shows green | [docker.com/products/docker-desktop](https://www.docker.com/products/docker-desktop/) |
| 2 | **Git** | `git --version` in a terminal | [git-scm.com](https://git-scm.com) |
| 3 | **Azure CLI** | `az --version` | [learn.microsoft.com/cli/azure/install-azure-cli](https://learn.microsoft.com/en-us/cli/azure/install-azure-cli) |
| 4 | **Key Vault Secrets User role** on `kv-diiac-vendorlogic` | Your admin grants this — see `customer-config/vendorlogic/keyvault-secrets-manifest.md` | — |
| 5 | **Access to the Vendorlogic Entra tenant** | Ask your Azure admin to confirm you have access | — |

> **Windows users:** All commands in this guide work in either **PowerShell** or **Git Bash**.
...
The DIIaC platform needs three secrets to run: an admin token, a signing key, and an OpenAI API key.

---

## Step 5: Pull secrets from Azure Key Vault
...
Sign in with your Vendorlogic work account. You should see your account details printed.
...
✅ You should see a table listing `diiac-admin-api-token`, `diiac-signing-private-key-pem`, etc.

> **If you see "does not have secrets list permission":** Ask your Azure admin to assign you the
> `Key Vault Secrets User` role on `kv-diiac-vendorlogic`.
...
**What happened:** `.env` and `.secrets/signing_key.pem` were created automatically.
...
The signing key wasn't loaded from `.secrets/signing_key.pem`.

Re-run the pull script and restart:
```bash
bash scripts/pull-keyvault-secrets.sh
