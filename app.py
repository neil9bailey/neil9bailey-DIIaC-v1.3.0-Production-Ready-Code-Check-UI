...
def _load_or_create_signing_key() -> tuple[Ed25519PrivateKey, str]:
    # Prefer direct file read — avoids $(cat ...) shell substitution which strips
    # the trailing newline from the PEM, causing OpenSSL 3.x to reject the key.
    ...
    if key_file and os.path.exists(key_file):
        ...
        if not isinstance(key, Ed25519PrivateKey):
            raise ValueError("SIGNING_KEY_FILE must contain an Ed25519 private key")
        return key, "configured"

    pem = os.getenv("SIGNING_PRIVATE_KEY_PEM")
    if pem:
        # Normalise line endings in case of CRLF or literal \n from env injection
        pem = pem.replace("\\n", "\n").replace("\r\n", "\n")
        if not pem.endswith("\n"):
            pem += "\n"
