...
Must include: `http://localhost:5174/auth/callback`

If missing:
```powershell
az ad app update --id b726558d-f1c6-48f7-8a3d-72d5db818d0f --spa-redirect-uris "http://localhost:5174/auth/callback" "http://localhost:5174"
```
...
```powershell
docker compose -p diiac_v121_codex -f docker-compose.yml -f docker-compose.staging.yml up --build
```

This repo is isolated by default with project `diiac_v121_codex` and host ports:
...
# - Port 8100 in use -> set RUNTIME_HOST_PORT=8101 in .env
```
...
# - Port 3101 in use -> set BRIDGE_HOST_PORT=3102 in .env
```
...
# - Port 5174 in use -> set FRONTEND_HOST_PORT=5175 in .env
```
...
## PHASE 4 — VALIDATE: Governance Runtime (Port 8100)
...
Invoke-RestMethod http://localhost:8100/admin/health -Headers $headers | ConvertTo-Json -Depth 5
```
...
```powershell
try { Invoke-RestMethod http://localhost:8100/admin/health } catch { $_.Exception.Response.StatusCode }
...
```powershell
(Invoke-RestMethod http://localhost:8100/api/business-profiles).Count
...
```powershell
Invoke-RestMethod http://localhost:8100/admin/metrics -Headers $headers | ConvertTo-Json -Depth 5
# Expected: health_status: "OK", alerts: []
```

**Checkpoint 4:** Runtime healthy, signing configured, admin auth enforced, 5 business profiles loaded.
...
Invoke-RestMethod -Method POST http://localhost:8100/api/human-input/role -ContentType "application/json" -Body $roleInput | ConvertTo-Json -Depth 5
```
...
$result = Invoke-RestMethod -Method POST http://localhost:8100/api/governed-compile -ContentType "application/json" -Body $compileInput
...
```powershell
Invoke-RestMethod http://localhost:8100/verify/execution/$execId | ConvertTo-Json -Depth 5
...
```powershell
Invoke-RestMethod http://localhost:8100/executions/$execId/merkle | ConvertTo-Json -Depth 5
...
```powershell
Invoke-RestMethod http://localhost:8100/decision-pack/$execId/export-signed | ConvertTo-Json -Depth 10
...
```powershell
Invoke-RestMethod http://localhost:8100/trust/status | ConvertTo-Json -Depth 5
...
$audit = Invoke-RestMethod -Method POST http://localhost:8100/admin/audit-export -ContentType "application/json" -Body $auditBody -Headers $headers
...
## PHASE 6 — VALIDATE: Backend-UI-Bridge (Port 3101)

### 6.1 — Bridge Health

```powershell
Invoke-RestMethod http://localhost:3101/health | ConvertTo-Json -Depth 5
...
```powershell
Invoke-RestMethod http://localhost:3101/auth/status | ConvertTo-Json -Depth 5
...
Invoke-RestMethod http://localhost:3101/api/business-profiles | ConvertTo-Json -Depth 5
# Expected: Same 5 profiles as direct runtime call
```
...
## PHASE 8 — VALIDATE: Frontend & Entra SSO (Port 5174)
...
4. You'll be redirected back to `http://localhost:5174/auth/callback`
...
- Check redirect URI is registered: `http://localhost:5174/auth/callback`
...
> start their own `app.py` subprocess on port 8000. If Docker is using host port
> 8100, either stop Docker first OR run only the pytest suite while Docker
> is running.
...
```powershell
# Stop Docker first if using host port 8100
...
| Service | URL | Auth |
|---------|-----|------|
| Frontend | http://localhost:5174 | Entra SSO (MSAL) |
| Bridge | http://localhost:3101 | Entra JWT (RS256) |
| Runtime | http://localhost:8100 | None (health), Bearer token (admin) |
...
- [ ] 5 business profiles loaded (Phase 4.4)
...
- [ ] Frontend loads at localhost:5174 (Phase 8.1)
