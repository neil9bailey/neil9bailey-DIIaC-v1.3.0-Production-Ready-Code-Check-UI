...
### First-time setup: Generate and upload signing key to Key Vault

If Key Vault does not yet contain `diiac-signing-private-key-pem`, generate and
upload one before running the pull script:
...
# Upload to Key Vault — ALWAYS use --file, not --value
...
> **Important:** Always use `--file`, not `--value`. PowerShell variable expansion
> and shell quoting both truncate multi-line PEM strings, producing a corrupted
> single-line value that the runtime cannot deserialise.
...
# Check signing key is a valid PEM with proper line breaks
cat .secrets/signing_key.pem
# Must show 3 separate lines:
#   -----BEGIN PRIVATE KEY-----
#   MC4CAQAwBQYDK2Vw...  (base64)
#   -----END PRIVATE KEY-----
# If all on one line → key is corrupt. See Troubleshooting: "ValueError: Could not deserialize key data"

# Verify it's Ed25519 (not RSA or EC)
openssl pkey -in .secrets/signing_key.pem -noout -text 2>&1 | grep -i ed25519
# Must show: ED25519 Private-Key:
```
...
> **IMPORTANT — use the SPA platform, NOT Web.**
> MSAL v5 uses Authorization Code + PKCE (a browser-based flow).
> Redirect URIs registered under the **Web** platform will cause a
> `400 Bad Request` on the token exchange because Microsoft expects a
> client secret for Web-platform redirects. The **SPA** platform tells
> Entra to accept PKCE instead.
...
5. If the same URIs exist under the **Web** platform section, **remove them**
6. Click **Save**

**CLI alternative** (if URIs are currently under Web):
```bash
# Move URIs from Web to SPA platform
...
### `ValueError: Could not deserialize key data` — Signing key wrong algorithm or corrupt

**Symptom:** `governance-runtime` crashes on startup with:
```
ValueError: SIGNING_PRIVATE_KEY_PEM is set but contains invalid key data ...
Ensure the value is a valid PEM-encoded Ed25519 private key.
```

**Root cause:** The key in `.secrets/signing_key.pem` (and/or Key Vault) is the
wrong algorithm (e.g. RSA, EC) or has corrupted formatting (all on one line,
wrong line endings). The runtime requires an **Ed25519 PKCS8** private key.

**Common causes:**
1. Key was generated with `openssl genrsa` (RSA) instead of `openssl genpkey -algorithm ed25519`
2. PowerShell pipeline stripped newlines when writing the PEM (all content on one line)
3. Key Vault was uploaded with shell expansion that truncated multi-line content

**Fix — Regenerate and re-upload (PowerShell):**

```powershell
# 1. Generate a new Ed25519 key — use Python to write the file directly
#    (avoids PowerShell pipeline stripping newlines)
python -c "
...
with open('.secrets/signing_key.pem', 'wb') as f:
    f.write(pem)
print('Done - Ed25519 key written to .secrets/signing_key.pem')
"

# 2. Verify the local file has 3 separate lines
Get-Content .secrets\signing_key.pem
# Expected output (3 lines):
#   -----BEGIN PRIVATE KEY-----
#   MC4CAQAwBQYDK2Vw...  (base64, ~44 chars for Ed25519)
#   -----END PRIVATE KEY-----

# 3. Upload to Key Vault using --file (not --value, which truncates on newlines)
az keyvault secret set `
    --vault-name "kv-diiac-vendorlogic" `
    --name "diiac-signing-private-key-pem" `
    --file .secrets\signing_key.pem

# 4. Verify Key Vault has the full PEM
az keyvault secret show `
    --vault-name "kv-diiac-vendorlogic" `
    --name "diiac-signing-private-key-pem" `
    --query "value" -o tsv
# Must show all 3 lines with BEGIN/base64/END
```

**Fix — Regenerate and re-upload (Bash / Mac / Linux / WSL):**
...
# 2. Verify algorithm
openssl pkey -in .secrets/signing_key.pem -noout -text
# Must show: ED25519 Private-Key:
...
> **Key lesson:** Always use `az keyvault secret set --file` for PEM values.
> The `--value` flag combined with PowerShell variable expansion silently
> truncates multi-line strings.
...
### MSAL login returns 400 Bad Request

This means the redirect URIs are under the **Web** platform instead of **SPA**.
MSAL v5 uses Authorization Code + PKCE which requires the SPA platform.

1. Go to the app registration → **Authentication**
2. Move URIs from **Web** to **Single-page application** (see Step 4)
3. Clear browser sessionStorage and try again

### MSAL redirect loop / login fails

1. Confirm `http://localhost:5173/auth/callback` is in the DIIaC-UI app registration
   redirect URIs under the **SPA** platform (not Web) — see Step 4.
...
4. Clear browser sessionStorage: `sessionStorage.clear()` in DevTools console.
