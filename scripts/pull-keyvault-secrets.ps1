...
$CUSTOMER_INSTANCE_ID = if ($env:CUSTOMER_INSTANCE_ID)  { $env:CUSTOMER_INSTANCE_ID }  else { Cfg "CUSTOMER_INSTANCE_ID"    "$Customer-prod" }
$SIGNING_KEY_ID_VALUE = if ($env:SIGNING_KEY_ID)        { $env:SIGNING_KEY_ID }        else { Cfg "SIGNING_KEY_ID"         "diiac-$CUSTOMER_INSTANCE_ID" }
$SIGNING_KEY_ID_VALUE = ($SIGNING_KEY_ID_VALUE.ToLower() -replace '[^a-z0-9._-]', '-')
...
try {
    ...
    if ($githubToken) {
        ...
        Write-Fail "diiac-github-token not found in Key Vault - Copilot is required in this release."
    }
} catch {
    Write-Fail "diiac-github-token not found in Key Vault - Copilot is required in this release."
}
...
# Ports (isolated defaults for v1.2.1 codex workspace)
...
Write-Ok "SIGNING_KEY_ID set to $SIGNING_KEY_ID_VALUE"
...
Write-Host "  docker compose -p diiac_v121_codex -f docker-compose.yml -f docker-compose.staging.yml up --build"
Write-Host ""
