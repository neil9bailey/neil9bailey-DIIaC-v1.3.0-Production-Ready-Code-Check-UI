...
CUSTOMER_INSTANCE_ID="${CUSTOMER_INSTANCE_ID:-${CUSTOMER}-prod}"
SIGNING_KEY_ID_VALUE="${SIGNING_KEY_ID:-diiac-${CUSTOMER_INSTANCE_ID}}"
SIGNING_KEY_ID_VALUE="$(echo "$SIGNING_KEY_ID_VALUE" | tr '[:upper:]' '[:lower:]' | sed 's/[^a-z0-9._-]/-/g')"
...
SIGNING_RAW=$(az keyvault secret show \
  --vault-name "$KV_NAME" \
  --name "diiac-signing-private-key-pem" \
  --query "value" -o tsv)

# Key Vault flattens PEM newlines to spaces on retrieval.
# Reconstruct proper 3-line PEM: BEGIN / base64 / END
printf '%s\n' "$SIGNING_RAW" \
  | sed 's/-----BEGIN PRIVATE KEY-----[[:space:]]*/-----BEGIN PRIVATE KEY-----\n/' \
  | sed 's/[[:space:]]*-----END PRIVATE KEY-----/\n-----END PRIVATE KEY-----/' \
  > .secrets/signing_key.pem
...
if az keyvault secret show --vault-name "$KV_NAME" --name "diiac-github-token" \
  ...
  fail "diiac-github-token not found in Key Vault - Copilot is required in this release."
fi
...
# ── Ports ─────────────────────────────────────────────────────────────────────
RUNTIME_HOST_PORT=8100
BRIDGE_HOST_PORT=3101
...
VITE_LLM_PROVIDER_MODE=copilot_only
...
VITE_ENTRA_REDIRECT_URI=http://localhost:5174/auth/callback
VITE_ENTRA_GROUP_MAP={"${ENTRA_ADMIN_GROUP_ID}":{"role":"admin"},"${ENTRA_USER_GROUP_ID}":{"role":"standard"}}
ENVEOF

chmod 600 .env
ok ".env written"
ok "SIGNING_KEY_ID set to ${SIGNING_KEY_ID_VALUE}"
...
echo ""
echo "Next step:"
echo "  docker compose -p diiac_v121_codex -f docker-compose.yml -f docker-compose.staging.yml up --build"
echo ""
