...
## Current status
- v1.2.0 runtime is fully operational for governed compile, deterministic outputs, merkle proofing, signed exports, and audit endpoints.
...
28. Azure Key Vault `kv-diiac-vendorlogic` is now provisioned (uksouth, RBAC model, soft-delete + purge protection enabled). Docs updated: `PRE-KEY VAULT` banners removed from `GETTING_STARTED.md`, `VENDORLOGIC_LOCAL_STAGING_GUIDE.md`, and `KEY_VAULT_TRANSITION.md`. Key Vault pull scripts (`scripts/pull-keyvault-secrets.sh` / `.ps1`) are the active secret path — no runtime or bridge code changes required (all secrets arrive as env vars). Remaining operator actions before first KV-backed stack run: populate the 3 secrets in Key Vault, assign `Key Vault Secrets User` RBAC roles (see `customer-config/vendorlogic/keyvault-secrets-manifest.md`), run pull script, then `docker compose -f docker-compose.yml -f docker-compose.staging.yml up --build`.
