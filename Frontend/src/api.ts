...
export interface GovernanceExecutionState {
  ...
  key_mode?: string;
  llm_contributions?: {
    chatgpt: LlmContribution | null;
    copilot: LlmContribution | null;
  };
}
...
export interface VerifyReplayResponse {
  status?: string;
...
export interface IntegrationsHealthResponse {
  ...
  llm_integration: {
    ...
    stub_mode: boolean;
    model: string;
    dual_llm_loop?: boolean;
    providers?: {
      chatgpt: { status: string; model: string; role: string };
      copilot: { status: string; model: string; role: string };
    };
  };
...
export interface TrendSummaryResponse {
  window_hours: number;
  timestamp: string;
  request_intercepts: { total: number; allow_pct: number; restrict_pct: number; require_approval_pct: number };
  response_governance: { total: number; allow_pct: number; remediate_pct: number };
...
export function runLlmGovernedCompile(payload: {
  ...
  assertions?: string[]; governance_modes?: string[]; provider?: string; human_intent?: string;
...
export interface BridgeLedgerLogsResponse {
  total: number;
  returned: number;
  ledger_root: string;
  frozen: boolean;
...
export interface BridgeTrustStatusResponse {
  ledger_records: number;
  ledger_root: string;
  frozen: boolean;
  ledger_append_enabled: boolean;
  source: string;
}
export function fetchBridgeLedgerLogs(): Promise<BridgeLedgerLogsResponse> { return request<BridgeLedgerLogsResponse>("/admin/ledger/logs"); }
export function fetchBridgeTrustStatus(): Promise<BridgeTrustStatusResponse> { return request<BridgeTrustStatusResponse>("/trust/status"); }
...
export function verifyReplay(payload: { execution_id: string }): Promise<VerifyReplayResponse> { return request<VerifyReplayResponse>("/verify/replay", { method: "POST", body: JSON.stringify(payload) }); }
export function verifyReplayBatch(payload: { execution_ids: string[] }): Promise<VerifyReplayResponse> { return request<VerifyReplayResponse>("/verify/replay/batch", { method: "POST", body: JSON.stringify(payload) }); }
